...
@dataclass
class SimulatingSummary:
    start: float          # us
    end: float            # us
    duration: float       # us
    memory_info: float    # bytes, peak live memory for this op
...
class Runner:
    ...
    def __init__(
        ...
        self.timings: Dict[Node, SimulatingSummary] = {}
        self.peak_memory: Dict[int, float] = {}  # rank -> bytes

    # ------------------------------------------------------------------ run

    def run(self) -> Dict[Node, SimulatingSummary]:
    ...
    def _step2_ideal_timeline(self) -> None:
        timings: Dict[Node, SimulatingSummary] = {}
        for node in nx.topological_sort(self.graph):
            preds = list(self.graph.predecessors(node))
            start = max((timings[p].end for p in preds), default=0.0)
            dur = self.durations[node]
            mem = self.op_results[node].peak_memory() if node in self.op_results else 0.0
            timings[node] = SimulatingSummary(start=start, end=start + dur, duration=dur, memory_info=mem)
