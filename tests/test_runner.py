...
from zrt.runner.runner import Runner, SimulatingSummary
...
def _dump(runner: Runner) -> None:
    """Pretty-print per-node OpResult + SimulatingSummary. Visible with `pytest -s`."""
    print()
...
def test_single_chain_topo_order(chip):
    ...
    assert timings[a] == SimulatingSummary(start=0.0, end=1.0, duration=1.0, memory_info=0.0)
    assert timings[b] == SimulatingSummary(start=1.0, end=3.0, duration=2.0, memory_info=0.0)
    assert timings[c] == SimulatingSummary(start=3.0, end=6.0, duration=3.0, memory_info=0.0)
